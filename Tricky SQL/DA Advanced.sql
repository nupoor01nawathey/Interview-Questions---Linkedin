-- https://www.youtube.com/watch?v=ZojpmvXiQXo

create database DA_advanced ;
use DA_advanced ;
...
-- 1. Find top 3 outlets by cuisine type wihout using limit and top function
with cte as 
(	
	select 
		cuisine, 
...
select cuisine, restaurant_id, number_of_orders from (
	select 
		*, 
		row_number() over(partition by cuisine order by number_of_orders desc) rn
	from cte 
) tmp
where rn <= 3
;
...
-- (everyday how many new customers we acquired, if a customer is considered for prev days ignore that customer)
-- below works only when repetative customer appears later than the new customer on the given day which is taken care by the dataset
with cte as (
select 
	customer_code,
    date_format(min(placed_at), '%Y-%m-%d') first_order_date
from orders
group by 1 
...
where last_order < date_add(last_order, INTERVAL -7 DAY)
and first_order < date_add(first_order, INTERVAL -30 DAY)
and orders.promo_code_name is not null
;


-- 5. Growth team is planning to create a trigger that will target customers after their EVERY 3rd order 
...
select * from cte 
where rn % 3 = 0
-- and cast(placed_at as date) = cast(current_date() as date)
;


-- 6. List customers who placed more than 1 order and all their orders on a promo code
select 
	customer_code,
	count(*) as number_of_orders,
...
-- 7. What percent of customers were organically acquired in Jan 2025 (meaning customers who placed their first order without promo code)
with cte as (
	select 
		*,
		row_number() over(partition by customer_code order by placed_at) rn
...
select 
	100.0 * count(case when rn=1 and promo_code_name is null then customer_code end)/count(distinct customer_code) customer_perc
from cte
;
